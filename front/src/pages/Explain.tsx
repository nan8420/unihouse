...
const Explain = () => {
  ...
  // const [likelength, setLikelength] = useState(singlePost?.Likers?.length);
  ...
  useEffect(() => {
    const firstuseEffect = async () => {
      const hi = await dispatch(loadPost({postId: itemId}));

      // 좋아요 상태
      const liked = hi.payload?.Likers?.find((v: any) => v.id === myid);

      if (liked) {
        setLike(true);
      }
      // 좋아요 개수
      setLikelength(hi.payload?.Likers.length);

      // console.log('hi:', hi.payload);
    };

    firstuseEffect();
  }, [itemId, dispatch, myid]);
        ...
        <View style={styles.first}>
          <View style={styles.abovecon}>
            <View style={styles.namedaycon}>
              <Text style={styles.name}>{singlePost?.User?.nickname}</Text>
          ...
          <View style={styles.contentcon}>
            <Text style={styles.contenttxt}>{singlePost?.content}</Text>
          </View>
          ...
          {singlePost?.Images[0]?.src && (
            <View style={styles.Imagecon}>
              <Image
                style={styles.Image}
                source={{
                  uri: `${Config.API_URL}/${singlePost?.Images[0]?.src}`,
                }}
              />
            </View>
          )}
